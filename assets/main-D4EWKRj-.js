...
`;function s(){let r=document.getElementById(`filters`);r.innerHTML=`
  ...
  `}function c(){let e=document.getElementById(`enquiry`);e.innerHTML=`
  ...
  `}s(),c(),a(),document.getElementById(`category-filter`).addEventListener(`change`,o),document.getElementById(`price-filter`).addEventListener(`change`,o),document.getElementById(`occasion-filter`).addEventListener(`change`,o);var l=[`Sending your message… waking people up…`,`Packaging your message with care 📦`,`Dispatching pigeons 🕊️`,`Talking to our servers… they're thinking 🤔`],u=[`Enquiry sent! Our team has been notified and is pretending to look very busy 😄`,`Boom! Message delivered. Humans will take over from here 🤝`,`Success! Someone on our team just said, ‘Ooo, a new enquiry!’ 👀`,`Your message landed safely. Gifting experts assembling as we speak 🎁`,`Done! Our team is now emotionally invested in your enquiry 😌`,`Enquiry received! Expect a call faster than relatives asking wedding dates 📞😄`,`All set! We’ve logged your request and nodded very seriously 👍`],d=[`Oops! Seems the internet blinked. Try once more?`,`Uh oh 😅 Our server tripped. Please try again!`,`That didn't go as planned… tech team mildly embarrassed 🙈`,`Message didn't make it this time 😔 Let's give it another shot!`],f=null;function p(e,t,n=25){f&&=(clearInterval(f),null),e.innerHTML=``;let r=0;f=setInterval(()=>{e.innerHTML+=t.charAt(r),r++,r>=t.length&&(clearInterval(f),f=null)},n)}function m(e){return e[Math.floor(Math.random()*e.length)]}function h(){let e=document.getElementById(`enquiry`);e.innerHTML=`
    <div class="enquiry-success">
      <h3>✅ Enquiry Sent!</h3>
      <p>${m(u)}</p>
    </div>
  `}document.getElementById(`enquiry-form`).addEventListener(`submit`,async e=>{e.preventDefault();let t=e.target.querySelector(`button[type="submit"]`),n=document.getElementById(`form-status`);t.disabled=!0,t.innerText=`Submitting...`;let r=0;p(n,l[r],20);let i=setInterval(()=>{r=(r+1)%l.length,p(n,l[r])},2e3),a={name:document.getElementById(`name`).value,phone:document.getElementById(`phone`).value,email:document.getElementById(`email`).value,occasion:document.getElementById(`enquiry-occasion`).value,budget:document.getElementById(`budget`).value,message:document.getElementById(`message`).value};try{let e=await fetch(`https://curatedgifitngservice-backend-production.up.railway.app/send-enquiry`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(a)});clearInterval(i),e.ok?h():(p(n,m(d)),t.disabled=!1,t.innerText=`Submit Enquiry`)}catch{clearInterval(i),p(n,m(d)),t.disabled=!1,t.innerText=`Submit Enquiry`}});