...
`;function s(){let r=document.getElementById(`filters`);r.innerHTML=`
  ...
  `}s(),c(),a(),document.getElementById(`category-filter`).addEventListener(`change`,o),document.getElementById(`price-filter`).addEventListener(`change`,o),document.getElementById(`occasion-filter`).addEventListener(`change`,o),document.getElementById(`enquiry-form`).addEventListener(`submit`,async e=>{e.preventDefault();let t={name:document.getElementById(`name`).value,phone:document.getElementById(`phone`).value,email:document.getElementById(`email`).value,occasion:document.getElementById(`enquiry-occasion`).value,budget:document.getElementById(`budget`).value,message:document.getElementById(`message`).value,timestamp:new Date().toISOString()};(await fetch(`https://curatedgifitngservice-backend-production.up.railway.app/send-enquiry`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(t)})).ok?(alert(`Enquiry sent successfully!`),e.target.reset()):alert(`Failed to send enquiry`)});